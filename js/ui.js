...
export function applyTheme(dark, save) {
  ...
  const label = dark ? 'CLAIR' : 'SOMBRE';

  // Bouton thème original (topbar match)
  const lbl = document.getElementById('themeLabel');
  const ico = document.getElementById('themeIcon');
  if (lbl) lbl.textContent = label;
  if (ico) ico.style.background = dark ? 'transparent' : 'rgba(255,255,255,0.15)';

  // Boutons thème adaptatifs (autres écrans) — sélection par classe
  document.querySelectorAll('.btn-theme-adaptive span').forEach(el => {
